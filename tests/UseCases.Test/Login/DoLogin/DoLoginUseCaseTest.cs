...
public class DoLoginUseCaseTest
{
	...
	private static DoLoginUseCase CreateUseCase(MyRecipeBook.Domain.Entities.User? user = null)
	{
		...
		var userReadOnlyBuilder = new UserReadOnlyRepositoryBuilder();
		var accessTokenGenerator = JwtTokenGeneratorBuilder.Build();

		if (user is not null)
			userReadOnlyBuilder.GetByEmailAndPassword(user);

		return new DoLoginUseCase(userReadOnlyBuilder.Build(), passwordEncripter, accessTokenGenerator);
	}
}
